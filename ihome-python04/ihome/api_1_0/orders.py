...
def get_user_orders():
    ...
    try:
        if 'landlord' == role:
            ...
            houser_ids = [house.id for house in houses]
            # 再查询预定了自己房子的订单
            orders = Order.query.filter(Order, user_id == user_id).order_by(Order.create_time.dece()).all()
        else:
            # 以房客的身材查询订单，查询自己的预定的订单
...
def accept_reject_order(order_id):
    ...
    try:
        ...
        current_app.app.logger.errno(e)
        db.session.rollback()
...
def save_order_comment(order_id):
    ...
    # 获取参数
    req_data = request.get_json()
    comment = req_data.get('commnet')  # 评价信息
    ...
    try:
        ...
        order = Order.query.filter(Order.id == order_id, Order.user_id == user_id,
                                   Order.status == 'WAIT_COMMENT').fitst()
        house = order.house
    except Exception as e:
        current_app.logger.error(e)
    ...
    try:
        # 删除redis信息
        redis_store.delete('jouse_info_%s' % order.house.id)
    except Exception as e:
        current_app.logger.error(e)
